...
**THE CHECKER CONFIRMS. IT DOES NOT DISCOVER (v2.1.17).** Build → verify → learn a rule the reference always
contained → fix → publish → verify again is guess-and-correct, and it is the pack's entire remaining cost: three
sections on one page cost 8 publishes, and the two most expensive corrections — a breakpoint that hides two of
three cards, and type sizes that had to be measured per width — were both sitting in the reference at intake.
Neither was a judgement call. **If a gate can find it, intake was supposed to know it.** So: capture every
breakpoint the build will ship, compile all of them, diff them into overrides, resolve fonts and assets, and
only then write — once. A verify run that changes the build is an intake bug; log it as one.
...
| 2b | **PLAN THE WHOLE SECTION FROM THE SOURCE — every breakpoint, before the first write** | `node "$WF/scripts/wf-plan.js" <url> "<selector>" --prefix=<block> --out=<specs dir> --fonts=<site's installed families>` — captures 1440/991/767/478, compiles each, and **diffs them into base + real breakpoint overrides automatically**, including every element the reference HIDES at a width. Also returns readiness: fonts the site must already have, image slots the owner must fill. Exit 1 = the build is not buildable yet; fix the blocker (usually a font or an asset) BEFORE writing, not after publishing. Hand-authoring a plan from a reading of the source is now a **process failure** — that is exactly how a 586-node extract became a 20-string plan, and how a mobile rule that hides two of three cards was learned by failing a gate instead of by reading the reference. |
...
| 3b | **MEASURE the reference (v2.1)** | `node "$WF/scripts/text-extents.js" contract <ref.png> --bands=<label>:<y0>:<y1>,… --xoff=<N> --out=specs/<section>.extents.json` — turns the render into numbers **before** the build: true ink left/right/width per text line. Those numbers are what the build must hit. Tracking is then **solved**, never derived: `text-extents.js solve --target= --measured= --ls= --gaps=`. Skipping this is what cost the example-header build four extra publishes (see § F). |
| 3c | **Cross-check the spec against the render — BLOCKING** | `node "$WF/scripts/text-extents.js" check-spec --measured=<px> --font-size=<px> --glyphs=<n> --ls=<px>[:<count>],…` — decomposes the measured ink into glyph advances + tracking. Exit 1 = the source's numbers are arithmetically impossible against the render, so **do not build from them**; solve instead. Needs no font metrics. On example-header the shipped spec implied **−27.9px of glyph advance** in a 111px line: caught in one call, before any write. |
| 4 | **Preflight the plan** | `node "$WF/scripts/wf-preflight.js" <plan.json> --site-prefix=<id> --known-prefixes=<from registry.md> --fonts=<installed families>` — validates every type, class, combo, longhand, **native-expressibility (Invariant 5)** and skeleton BEFORE a single MCP write. `--fonts` makes the font check exact: a family the site does not have is a BLOCKER, not a fallback. Exit 1 = fix the plan, do not build. |
...
**The loop is the cost (v2.1).** `wf-section verify` records verdict+score; a verify that reproduces the previous one closed nothing, and two of those prints **STALLED** — at which point the next step is a *measurement*, never another fix-and-publish. Measured on example-header: 6 publishes and 4 verify runs against the then-budget of 15 calls, every one of them because the build targeted spec arithmetic instead of the render.
...
**The MCP-side call floor is ~9, not ~14 — three measured sources of waste (v2.1).** (This counts the MCP/build calls only; the ~25-call section budget also covers the Bash pipeline steps 0-4b, 6b-7 and 9.)
- **Load the MCP surface in ONE `ToolSearch`.** Fetch the standard Webflow set together (`webflow_guide_tool, data_agent_instructions_tool, data_sites_tool, data_pages_tool, data_element_tool, data_element_builder, data_element_settings_tool, data_style_tool, data_assets_tool, data_fonts_tool, element_snapshot_tool, designer_tool`) at the first build call of the session. Fetching them piecemeal cost 3-4 calls on example-header.
...
**A global pixel percentage cannot see a small text run.** On example-header an entire missing line ("OF FRAGRANCES", ~10px tall in a 1632×117 bar) scored **98.75% PASS with zero hot regions** — under the 3% budget and diluted across the 12×12 cell grid. The eye caught it in one view; the differ never would. So: the anchor view stays mandatory, and every text line the section owns gets a `text-extents` band (pixel-verify §1.4). **Δwidth is tracking, Δleft is position** — both are numbers you can fix from.
**Image cost is ~1,500 tokens, maximum, at any resolution** (Anthropic bills `(w×h)/750` after a 1568px-edge downscale). Earlier pack versions claimed 5k-66k and rationed looking — that was wrong by ~40× and it cost accuracy. Budget: reference render always · one anchor compare against the built shot at the primary width even on PASS · the diff PNG on any FAIL or UNSCORED width. A PASS score is never a substitute for the anchor view — a global percentage cannot see a small text run, and property equality cannot see an element that renders empty (§F). Re-open a PASS width whenever the anchor view raises a question. That is 2-5 views per section, ~6k tokens, and it is the cheapest accuracy in the pack.
...
- **Longhand only, AND native-expressible.** `gap` → `grid-column-gap` + `grid-row-gap`; `border-radius` → all four corners. Full table in `build-reference` § Longhand; enforced by `wf-preflight`.
- **Invariant 5 has three failure modes, not one (v2.1.16).** `data_style_tool` accepts ANY css, but whatever the Designer has no field for is stored as a **Custom Property**: it emits CSS, so the pixel score, `dom-contract` property equality and a11y all pass — while the build is not native and the client cannot edit it in the Designer. The three: (a) a **shorthand**, (b) a **value function** — `clamp()` / `min()` / `max()` / `env()`, because a Webflow size field is one value + one unit, (c) a **value the panel cannot pick** — a `font-family` *stack* instead of one family installed on the site, or a property with no control at all (`aspect-ratio`, `inset`, `place-items`, `text-wrap`). All three are machine-checked by `wf-preflight` against `scripts/native-props.json`. Author what the Designer would write: one value + one unit at base plus **real breakpoint overrides measured from the reference at each width** (more accurate than a clamp curve, because it is what the reference actually renders there), longhand insets, native height or a padding-top ratio box instead of `aspect-ratio`.
- **A font that is not installed on the site is a BLOCKER, never a fallback stack.** A substituted face changes every glyph advance, so the section can never reach the pixel floor and no score can attribute the gap. Pass the site's installed families to preflight (`--fonts=`); if the reference's face is missing, hand it to the user (Site Settings → Fonts) and say so in one line.
...
shipping a value the Designer cannot hold (`clamp()`/`min()`/`max()`, a font fallback stack, `aspect-ratio`, `inset`) · substituting a font the site does not have instead of asking for it ·
...
## F. What the example-header build actually cost (2026-08-01) — read this before the next section
...
|---|---|---|---|
| assistant turns | 235 | 145 | ≤35 |
| tool calls | 107 | 68 | ≤25 |
