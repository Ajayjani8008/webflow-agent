...
**The loop is the cost (v2.1).** `wf-section verify` records verdict+score; a verify that reproduces the previous one closed nothing, and two of those prints **STALLED** — at which point the next step is a *measurement*, never another fix-and-publish. Measured on kush-header: 6 publishes and 4 verify runs against the then-budget of 15 calls, every one of them because the build targeted spec arithmetic instead of the render.
