...
- **Rejects `-webkit-*` prefixed properties.** For gradient text, FIRST try the unprefixed longhands on a nested `span` inside the heading: `background-image: <gradient>` + `background-clip: text` + `color: transparent` (worked in a production build — see memory [[webflow-pixel-match-method]]). Tool rejects those too → solid fallback color (mid-gradient hue) + ledger entry — Designer has native text-gradient picker (Text color → gradient). Never inject style.
...
| `designer_tool` fails on the FIRST call of a site | Prerequisite, not a bug: Designer tools reach the canvas through the bridge app running inside the Designer, and the authorized Webflow account must be the one whose Designer session is open. Ask for: Designer → Apps → search `MCP` → `Webflow MCP Bridge App` present under **Connected** and launched (installed per site via the marketplace app `Claude MCP Connector`) · Designer tab open on the target page · same account on both sides. Client site on another account and no login → build in the user's own site under `/portable on`, then hand over the paste + portability report (see `portable-mode`). Never diagnose this as a build failure |
