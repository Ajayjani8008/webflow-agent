...
5. **MCP FIRST — always, and know its CURRENT surface.** The Webflow MCP connector (Data + Designer tools) is faster and more capable than REST — the ONLY path when present. REST fallback exclusively when MCP tools are absent; never mix per-call (REST can't set class styles at all — build-reference § API fallback). **Session preamble, once, in order:** `webflow_guide_tool` (server requires it first + reports the MCP version) → explicit `site_id`, never assumed → `data_agent_instructions_tool > search_instructions` for that site + read every hit (site-owned rules/skills layer UNDER these rules; re-run on every site switch) → resolve page/branch. **Never claim a capability doesn't exist from memory** — `get_more_tools` with the category + a concrete brief settles it, and the answer is logged once in `error_learnings.md` with its date. Current surface, action names, breakpoint ids and the component/props/variants/slots tooling: build-reference § MCP surface — a repeated block is a **component with props**, not N copied subtrees, and component work is API-buildable now (no Designer handoff for that).
...
16. **HTML/URL REFERENCE = BEHAVIOUR CONTRACT, NOT A LAYOUT PICTURE.** An HTML delivery (or live-URL reference) is only "read" once **every** file it pulls in has been read end to end — markup, every local stylesheet and script, inline `<style>`/`<script>`, and every CDN library (design-intake §C.0). Then **run it**: headless via `file://` (or the live URL) for state shots + a motion fingerprint (§C.6) — hover, focus, click, scroll and load states, with exact durations, delays, easing curves, iteration counts and trigger thresholds (§C.5). Every hover · `::before`/`::after` · transition · transform · `@keyframes` · canvas · cursor/mouse effect · loading/reveal/scroll animation · custom JS behaviour enters the manifest and ships at its tier — libraries are routed natively (§C.2b: GSAP/AOS→Interactions panel, Swiper→native slider, Lottie-web→Lottie element, three/particles→T4 canvas), never re-injected, never dropped because "it was a library". **Verification is per state, measured:** pixel-verify §1.8 scores built-vs-reference at each state and fails a dead hover, a reveal that never fires, a frozen canvas or timing off by >10% — even at a perfect resting score. Reference unrunnable → say so, mark `reference-not-run`, verify the manifest on the built side; never skip it silently.
...
**Phase 2 — Intake:** design-intake per section (live-URL → url-intake). **NODE-FIRST:** node id in hand → `get_design_context` on it directly; NEVER `get_metadata` on page root (`0:1`) or whole page frame (~1MB, blows budget); climb ONE parent at a time. **FETCH-ONCE:** check `figma-cache/03-nodes/{node}.json` first; miss → one live fetch → write cache + screenshot + manifest immediately. Second fetch of a cached node = bug. Screenshot source → confidence levels. **HTML/URL → full-source read + library sweep + reference run (state shots · motion fingerprint · computed CSS) BEFORE the manifest** (design-intake §C.0/§C.6, Rule 16), then validate against Webflow property support.
...
| HTML/CSS delivery | design-intake §C — inventory the folder → read EVERY html/css/js file end to end → library sweep → **run it headless (`file://`) for state shots + motion fingerprint** → numbered manifest with exact timings → build every row at its tier → pixel-verify §1.8 per-state parity. Hover/`::before`/`::after`/transitions/`@keyframes`/canvas/cursor/preloader/scroll-reveal all preserved, never approximated |
