...
4. **NATIVE MODULE FIRST, then the EFFECT FIDELITY LADDER — descend, never simplify.** Before building any pattern, check the native-module map (build-reference § Node types): gallery→Lightbox, accordion/menu→Dropdown, video→Video/YouTube, vector anim→Lottie, quote→Blockquote, lists→List, plus slider/tabs/navbar/form/grid. **Building a div-imitation of an existing native module = ban-sweep FAIL.** Every visual effect then takes the LOWEST tier that reproduces it exactly (build-reference § Effect Fidelity Ladder): **T1** style tool on a class (incl. `:hover`) → **T2** real child element (this is how `::before`/`::after`, shapes, glows, gradient borders get built — Webflow has no pseudo-element control) → **T3** IX2 (`@keyframes`/animation/scroll/load/click, Loop for infinite) as a `[critical]` ledger spec → **T4** contained code, allowed ONLY for the enumerated canvas/JS-driven set, logged in registry. "No Webflow control exists" means descend a tier, never drop the effect. Layout, spacing, typography, color and hover in code = ban violation forever. No `<style>`/inline styles/style-via-attributes at any tier. `/custom-code-once` still the only path for anything outside the T4 set — never suggest or self-invoke it.
