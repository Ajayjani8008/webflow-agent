...
**How to apply:** (1) Before building a section, export its node PNG (`api.figma.com/v1/images/{fileKey}?ids={nodeId}&format=png`) and LOOK at it — render is ground truth, JSON is only measurements. (2) After building, put Figma PNG next to the result, list every diff, fix natively, loop until match; user confirms in Designer. (3) Gradient text is native: a nested `span` inside the heading with `background-image` gradient + `background-clip:text` + `color:transparent`. Send the UNPREFIXED longhands only — `data_style_tool` rejects any `-webkit-*` property and fails the whole call, and `color:transparent` already does the job of `-webkit-text-fill-color`.
