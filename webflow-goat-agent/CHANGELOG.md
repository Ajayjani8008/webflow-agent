...
from § F: on example-header a section scored **98.75% PASS, zero hot regions, `dom-contract` 158/158 property-equality
...
check is green. Separately, `wf-preflight --self-test` exited 1 on its site-initials case: it asserted
a prefix passes for a site whose initials do not produce it — a **sanitization bug** from
...
- **check-spec** (`text-extents.js`) — decomposes measured ink into glyph advances + tracking, so a
  ...
  shipped example-header spec implied -27.9px of glyph advance in a 111px line: caught in one call.
