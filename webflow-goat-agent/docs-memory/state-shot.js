...
//     auto                     hover every interactive descendant (max 6, DOM order)
...
(async () => {
  ...
  for (const st of requested) {
    ...
    try {
      ...
      if (kind === 'auto') {
        const targets = JSON.parse(await evalJS(
          `(function(){var r=document.querySelector(${JSON.stringify(root)});if(!r)return "[]";
            ...
            return JSON.stringify(out.slice(0, ${AUTO_MAX}))})()`) || '[]');
        for (const sel of targets) {
          const c = await centerOf(sel); if (!c) continue;
  ...
  const index = { url, root, width: W, mobile: mob, states: shots, blankWarning: shots.filter(s => s.bytes && s.bytes < 7000).map(s => s.state) };
