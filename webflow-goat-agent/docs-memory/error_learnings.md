...
- [2026-07-28] a11y: first `page-audit.js` run against a real published page (a real published staging site) found 7 genuine contrast failures at **3.86:1** on `.faq__question` (white on `rgba(149,119,162,.95)`) that every previous pixel-perfect pass had scored as PASS — pixel fidelity and accessibility are independent axes. Also: exclude `.w-webflow-badge` from any audit; Webflow's own chrome is unfixable and produces a permanent false FAIL.
